...
DEVICE_OUT = 4                              # Device ID of output device
...
def get_level(audio_data):
    ...
    if channel <= DEVICE_CHANNELS: # single channel
...
def plot_and_save_fft():
    global monitor_channel, FFT_GAIN, timestamp
    ...
    while not stop_fft_periodic_plot_event.is_set():
        ...
        myrecording = sd.rec(int(N), samplerate=SAMPLE_RATE, channels=DEVICE_CHANNELS)
        sd.wait()  # Wait until recording is finished
        myrecording *= gain
        ...
        # Ensure that bucket_size is at least 1
        bucket_size = max(1, bucket_size)

        # Pad yf and xf with last value to make their length a multiple of bucket_size
        yf = np.pad(yf, (0, -len(yf) % bucket_size), 'edge')
        xf = np.pad(xf, (0, -len(xf) % bucket_size), 'edge')
...
'''
        # If the last bucket isn't a full bucket, we remove it
        if len(buckets) != len(bucket_freqs):
            if len(buckets) > len(bucket_freqs):
                buckets = buckets[:-1]
            else:
                bucket_freqs = bucket_freqs[:-1]
...
'''
        ...
        output_filename = f"{timestamp}_fft_{SAMPLE_RATE/1000:.0F}_{BIT_DEPTH}_{monitor_channel}\
            _{LOCATION_ID}_{HIVE_ID}.png"
...
def plot_oscope_audio(): 
    global monitor_channel, DEVICE_IN, DEVICE_OUT
    ...
    # Record audio
    print("Recording...")
    myrecording = sd.rec(int(SAMPLE_RATE * TRACE_DURATION), samplerate=SAMPLE_RATE, channels=monitor_channel)
    sd.wait()  # Wait until recording is finished
    print("Recording finished.")
...
def show_audio_device_list():
    ...
    print("\nDevice list:")
    print(f"\nDevice_in: {DEVICE_IN}", show_audio_device_info(DEVICE_IN))
    print(f"Device_out: {DEVICE_OUT}", show_audio_device_info(DEVICE_OUT))
    print("\ncomplete list:")
...
def save_period_audio():
    ...
    output_filename = f"{timestamp}_period_{SAMPLE_RATE/1000:.0F}_{BIT_DEPTH}_{DEVICE_CHANNELS}\
...
def audio_stream():
    ...
    with stream:
        ...
        if MODE_FFT_PERIODIC_RECORD:
            ...
            fft_periodic_plot_thread.daemon = True  
...
def stop_all():
    ...
    if intercom_thread is not None and intercom_thread.is_alive():
    ...
    if fft_periodic_plot_thread is not None and fft_periodic_plot_thread.is_alive():
    ...
    print("is continuous_save_thread alive:", continuous_save_thread.is_alive())
    
    if continuous_save_thread.is_alive():
    ...
    if period_save_thread is not None and period_save_thread.is_alive():
    ...
    if event_save_thread is not None and event_save_thread.is_alive():
