...
# Parameters
sample_rate = 192000
frames_per_buffer = 1024  # Number of frames processed at a time
...
def worker_ffmpeg(duration, wait, thread_id, downsample=False):
    while True:
        ...
        if downsample:
            ...
            audio = AudioSegment(audio_data.tobytes(), frame_rate=downsample_rate, sample_width=audio_data.dtype.itemsize, channels=4)
...
# Start recording
with sd.InputStream(callback=callback, channels=DEVICE_CHANNELS, samplerate=sample_rate, frames_per_buffer=frames_per_buffer):
    # Worker threads
