...
class EventRecorder:
    def __init__(self, device=DEVICE_IN, rate=SAMPLE_RATE, channels=CHANNELS, duration=TIME_BEFORE + TIME_AFTER):
        ...
        self.post_event_countdown = 0  # Initialize a countdown for the time after event
    ...
    def callback(self, indata, frames, time, status):
        ...
        if volume_norm > self.threshold:
            ...
            self.post_event_countdown = TIME_AFTER  # Start the countdown (in seconds)
    ...
    def start_recording(self):
        ...
        try:
            while True:
                ...
                if self.triggered and self.post_event_countdown <= 0:  # Check if event happened and if post-event countdown has finished
                    ...
                    # scale the float data to int16 before writing the file
                    audio_data = np.array(self.buffer)
                    audio_data = np.int16(audio_data * 32767)
                    sf.write(output_path, audio_data, self.rate, format=FORMAT, subtype=BIT_DEPTH_IN)                    
                    print('* Finished saving')
                    self.buffer.clear()  # Clear the buffer for the next event
                elif self.post_event_countdown > 0:  # If post-event countdown is still ongoing, reduce it
                    self.post_event_countdown -= 0.1  # Decrease the countdown by the sleep duration
