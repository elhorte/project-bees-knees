...
class EventRecorder:
    def __init__(self, device=DEVICE_IN, rate=SAMPLE_RATE, channels=CHANNELS, duration=TIME_BEFORE + TIME_AFTER):
        ...
        self.buffer = []  # Using list instead of deque
        self.threshold = 10**(EVENT_TRIGGER/20)  # Convert dBFS to linear scale
        self.triggered = False
        self.time_of_trigger = None  # Initializing time_of_trigger attribute
    ...
    def callback(self, indata, frames, time_info, status):
        self.buffer.append(indata)  # Add the incoming data into the buffer
        if len(self.buffer) > (TIME_BEFORE + TIME_AFTER) * self.rate:  # If buffer size exceeds the maximum length
            self.buffer.pop(0)  # Remove the oldest audio data from buffer
        ...
        if volume_norm > self.threshold:
            ...
            self.time_of_trigger = time.time()  # set the time_of_trigger when event is detected
