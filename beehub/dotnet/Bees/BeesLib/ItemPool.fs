...
type ItemPool<'Item when 'Item :> IPoolItem>(startCount: int, minCount: int, creator: unit -> 'Item) =
  ...
  let changeAvail n = Volatile.Write(countAvail, Volatile.Read(countAvail) + n)
  let changeInUse n = Volatile.Write(countInUse, Volatile.Read(countInUse) + n)
  ...
  let takeFromPool() =
    ...
    if ok then
      changeAvail -1
      changeInUse +1
      assert (Volatile.Read countAvail = pool.Count)
      Some obj
    else
      None
  ...
  let createAndAddNewItems n =
    if n > 0 then
      ...
      assert (Volatile.Read(countAvail) = pool.Count)
      Volatile.Write(countInUse, 0)

  let addMoreItemsIfNeeded() = createAndAddNewItems (minCount - Volatile.Read countAvail)

        
  do
    if startCount <> 0 then  // see CbMessage constructor
      // Stock the pool.
      assert (Volatile.Read countAvail = 0)
      assert pool.IsEmpty
  ...
  member this.CountAvail               = Volatile.Read countAvail
  member this.CountInUse               = Volatile.Read countInUse
