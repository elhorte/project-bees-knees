...
type FrameRange = {
  ...
  member fr.Check() =
    ...
    assert (fr.IndexBegin = int (round ((fr.TimeBegin - fr.TimeOrigin) / frameDuration)))
    assert (fr.IndexEnd   = int (round ((fr.TimeEnd   - fr.TimeOrigin) / frameDuration)))
