            ...
            try:
                with sd.InputStream(callback=callback_input,
                    ...
                    while not stop_vu_queue.get():
                        sd.sleep(1000)  # Changed from 0.1 to 1000 milliseconds
            except Exception as e:
...
def toggle_intercom_m():
    ...
    if intercom_proc is None:
        ...
        try:
            ...
            intercom_proc.start()
            intercom_proc.join(timeout=1)  # Wait for the process to initialize
            
            # Check if the process is alive and handle errors
            if not intercom_proc.is_alive():
                raise RuntimeError("Intercom process failed to start")
