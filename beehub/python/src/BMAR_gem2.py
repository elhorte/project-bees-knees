...
# #############################################################
# #### Application Constants (Safe to remain global) ########
# #############################################################
...
def get_app_instance():
    """Get the global application instance. Used for signal handlers and callbacks."""
    global app_instance
    if app_instance is None:
        raise RuntimeError("Application instance not initialized. Call this after main() starts.")
    return app_instance
...
def signal_handler_with_app(sig, frame):
    """Signal handler that uses the global app instance."""
    try:
...
# (Removed duplicate global check_and_create_date_folders function. Use BmarApp.check_and_create_date_folders instead.)
# NOTE: Do not use any global check_and_create_date_folders function; always use BmarApp.check_and_create_date_folders for directory setup.
...
def find_file_of_type_with_offset(offset, directory=PRIMARY_DIRECTORY, file_type=config.PRIMARY_FILE_FORMAT):
    ...
    if not os.path.exists(expanded_dir):
        print(f"Directory does not exist: {expanded_dir}")
def find_file_of_type_with_offset_1(app, directory=None, file_type=None, offset=0):
    """
    Fetch the most recent audio file in the directory using app instance attributes.
    """
    if directory is None:
        directory = app.PRIMARY_DIRECTORY
    if file_type is None:
        file_type = app.config.PRIMARY_FILE_FORMAT
...
def find_file_of_type_with_offset(app, offset, directory=None, file_type=None):
    """
    Return the most recent audio file in the directory minus offset using app instance attributes.
    """
    if directory is None:
        directory = app.PRIMARY_DIRECTORY
    if file_type is None:
...
def plot_spectrogram(app, channel, y_axis_type, file_offset, period):
    ...
    Generate a spectrogram from an audio file and display/save it as an image.
    Parameters:
    - app: BmarApp instance for directory and config access
    - channel: Channel to use for multi-channel audio files
    - y_axis_type: Type of Y axis for the spectrogram ('log' or 'linear')
    - file_offset: Offset for finding the audio file
    """
...
def plot_and_save_fft(stop_event, config):
    ...
    while not stop_event.is_set():
        try:
            ...
            # Average buckets - ensure both arrays have the same length
def plot_and_save_fft(app, stop_event, config):
    """
    This function will be run in a separate thread.
    It will periodically record audio, generate an FFT plot, and save it to a file.
    """
    while not stop_event.is_set():
        try:
            ...
            full_plot_path = os.path.join(app.PLOT_DIRECTORY, plotname)
