...
sound_in_samplerate = 192000                # default, updated by sample rate of input device
...
def show_audio_device_list():
    ...
    print(f"\nCurrent device in: {sound_in_id}, device out: {config.SOUND_OUT_ID_DEFAULT}\n")
...
def on_press(key):
    try:
        if hasattr(key, 'char'):  # Single-character keys
            if key.char == 'c':
                ...
                # Stop the listener to exit
                return False
    except Exception as e:
        print(f"An error occurred while processing key press: {e}")
...
def main():
    ...
    try:
    ...
    except KeyboardInterrupt:  # Ctrl-C in Linux
        ...
        if KB_or_CP == 'KB' and listener.is_alive():
